...
opener.addheaders = [
    (
        "User-Agent",
        "mpv_sponsorblock/1.0 (https://github.com/po5/mpv_sponsorblock)",
    )
]
urllib.request.install_opener(opener)

if sys.argv[1] == "ranges" and (
    not sys.argv[2] or not os.path.isfile(sys.argv[2])
):
    sha = None
    if 3 <= int(sys.argv[6]) <= 32:
        sha = hashlib.sha256(sys.argv[4].encode()).hexdigest()[
            : int(sys.argv[6])
        ]
    times = []
    try:
        response = urllib.request.urlopen(
            sys.argv[3]
            + "/api/skipSegments"
            + ("/" + sha + "?" if sha else "?videoID=" + sys.argv[4] + "&")
            + urllib.parse.urlencode(
                [("categories", json.dumps(sys.argv[5].split(",")))]
            )
        )
        ...
        for segment in segments:
            ...
            if sha:
                ...
                times.append(
                    str(segment["segment"][0])
                    + ","
                    + str(segment["segment"][1])
                    + ","
                    + segment["UUID"]
                    + ","
                    + segment["category"]
    ...
    for category in sys.argv[5].split(","):
        c.execute(
            "SELECT startTime, endTime, votes, UUID, category FROM sponsorTimes WHERE videoID = ? AND shadowHidden = 0 AND votes > -1 AND category = ?",
            (sys.argv[4], category),
        )
        sponsors = c.fetchall()
        best = list(sponsors)
        ...
        for sponsor_a in sponsors:
            for sponsor_b in sponsors:
                if (
                    sponsor_a is not sponsor_b
                    and sponsor_a["startTime"] >= sponsor_b["startTime"]
                    and sponsor_a["startTime"] <= sponsor_b["endTime"]
                ):
    ...
    try:
        urllib.request.urlretrieve(
            sys.argv[3] + "/database.db", sys.argv[2] + ".tmp"
        )
    ...
    try:
        if sys.argv[6]:
            urllib.request.urlopen(
                sys.argv[3] + "/api/viewedVideoSponsorTime?UUID=" + sys.argv[5]
            )
        if sys.argv[9]:
            urllib.request.urlopen(
                sys.argv[3]
                + "/api/voteOnSponsorTime?UUID="
                + sys.argv[5]
                + "&userID="
                + uid
                + "&type="
    ...
    try:
        data = urllib.parse.urlencode(
            {"userID": uid, "userName": sys.argv[9]}
        ).encode()
        req = urllib.request.Request(
            sys.argv[3] + "/api/setUsername", data=data
        )
